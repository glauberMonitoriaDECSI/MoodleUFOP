...
transformaDisciplinas disciplinasFile = do
    ...
    case decodeByNameWith opcoesDec disciplinasCSVData of
        ...
        Right (_, vector) -> do
            disciplinasSaidas <- V.forM vector $ \ elemVetor -> do
                let
                    ...
                    fullN = disciplinaD elemVetor
                                ...
                                `T.append` " - "
                                `T.append` T.toTitle (professoresD elemVetor)
                    cat = departamentoD elemVetor
                            `T.append` " / "
            ...
            let outputFile = T.replace "/" "_" $ T.filter (/= ' ') (categoryD ((V.toList disciplinasSaidas)!!0)) `T.append` ".csv"
